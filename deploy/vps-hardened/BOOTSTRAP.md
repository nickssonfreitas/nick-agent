...
`DEPLOY.md` describes the whole deployment as one linear procedure. Once you
...
The reason `hermes login` never moves into CI is `DEPLOY.md` step 5: it writes
...
- The encrypted volume from `DEPLOY.md` step 1. Do this first — Hermes stores
...
`REPLACE_WITH_PINNED_DIGEST`. Do `DEPLOY.md` step 2 by hand once.

## Step 3 — Generate the dashboard password hash

`DEPLOY.md` step 3, unchanged. Interactive `getpass`, and it stays that way.

## Step 4 — Fill in `.env` and the Caddyfile

`DEPLOY.md` step 4. Two invariants CI will check but cannot fix for you:
...
`DEPLOY.md` step 5: `docker exec -it hermes hermes login`. This is the step
...
`DEPLOY.md` step 6, then hand-edit the `mcp_servers` block for your actual
...
`DEPLOY.md` makes the same point and it is worth repeating here, because a
pipeline that writes a new one on every deploy will quietly accumulate them.

## Residual risk

Automating the deploy does not change the risk posture in `DEPLOY.md`, and it
