---
type: Surface
title: Dashboard and web
description: The browser dashboard and the PTY bridge that embeds the real TUI inside it.
resource: web
tags: [surface, web, pty]
status: stable
sources:
  - id: repo
...
- **Anything asserting about `.ts`/`.tsx`, `package.json` or lockfiles belongs in
  vitest**, not in `tests/*.py`. See [Testing and CI](../operations/testing-and-ci.md).
...
[TUI](tui.md) · [Desktop](desktop.md) · [CLI](cli.md) · [Architecture](../concepts/architecture.md) · [Index](../index.md)
