...
description: The SQLite session store, FTS5 search and retention.
resource: hermes_state.py
tags: [state, sqlite, persistence]
status: stable
sources:
  - id: repo
...
writes. Persistence only; for how the gateway routes a message to a session see
...
The gateway's routing model — session keys, multi-user isolation, reset policy,
restart recovery — is a separate subsystem and lives in
[Session lifecycle](session-lifecycle.md). This page stops at persistence.
...
`clear`. A longer narrative lives in [Session lifecycle](session-lifecycle.md).
...
[Agent core](../core/agent-core.md) · [Memory and context](memory-and-context.md) · [Config and profiles](config-and-profiles.md) · [Index](../index.md)
