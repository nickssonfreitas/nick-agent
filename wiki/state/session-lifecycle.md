...
description: How the gateway derives a session key, isolates users, resets, recovers after restart and expires idle sessions. The routing half; persistence lives in sessions.md.
resource: gateway/session.py
tags: [state, sessions, gateway, isolation]
status: stable
sources:
  - id: repo
  ...
  - id: origin
    resource: docs/session-lifecycle.md
...
Owned by `gateway/session.py`, `gateway/run.py` and `gateway/config.py`.

**Scope boundary.** This page owns the *routing* half of a session: how the gateway
decides which conversation an inbound message belongs to, and when that conversation
resets, recovers or expires. It does not own persistence. The SQLite store, its
schema, FTS5 search and retention live in [State and sessions](sessions.md).
