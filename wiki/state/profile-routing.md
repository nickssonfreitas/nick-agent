---
type: Subsystem
title: Profile-based routing for inbound messages
description: How an inbound message picks the profile it runs under, and what happens when the mapping is missing or ambiguous.
resource: gateway/profile_routing.py
tags: [state, profiles, routing, gateway]
status: stable
sources:
  - id: repo
  ...
  - id: origin
    resource: docs/profile-routing.md
...
Owned by `gateway/profile_routing.py`, with `_profile_name_for_source` in
`gateway/run.py`, `build_source` in `gateway/platforms/base.py`, and the schema in
`gateway/config.py`. Sibling reading: [Session lifecycle](session-lifecycle.md) for
how the chosen profile then keys a session, and
[decision 0001](../decisions/0001-profile-builder-in-dashboard.md) for where profiles
are authored.
