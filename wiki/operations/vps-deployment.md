---
type: Process
title: Hardened VPS deployment
description: "The linear procedure for putting this fork on an internet-facing VPS: disk encryption, digest pinning, dashboard auth, reverse proxy and verification."
resource: deploy/vps-hardened/docker-compose.yml
tags: [operations, deployment, vps, security]
status: stable
sources:
  - id: repo
  ...
  - id: origin
    resource: deploy/vps-hardened/DEPLOY.md
...
The bundle itself lives at [`deploy/vps-hardened/`](../../deploy/vps-hardened) in the
repo; this page is the procedure that drives it. Paths below are relative to that
directory.
...
- **MCP stdio servers run unsandboxed** with your user's filesystem access. For
  ...
  isolation described in [Network egress isolation](network-egress-isolation.md) (not part
  of this bundle).
