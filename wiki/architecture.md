...
|---|---|---|---|
| Interactive CLI | `hermes` → `cli.py` | in-process `AIAgent` | [CLI](cli.md) |
| Messaging gateway | `hermes gateway` → `gateway/run.py` | in-process `AIAgent` per session | [Gateway](gateway.md) |
| Ink TUI | `hermes --tui` | `tui_gateway/` over stdio JSON-RPC | [TUI](tui.md) |
| Desktop app | Electron | `hermes serve` over WebSocket JSON-RPC | [Desktop](desktop.md) |
| Browser dashboard | `hermes dashboard` | `hermes_cli/web_server.py` + PTY-embedded TUI | [Dashboard and web](dashboard-web.md) |
| Editor (VS Code / Zed / JetBrains) | ACP | `acp_adapter/` | [MCP and ACP](mcp-and-acp.md) |
...
2. **Guards.** Two sequential guards decide whether the message reaches the agent at
   ...
   **both**. See [Gateway § The two guards](gateway.md#the-two-message-guards).
3. **Session resolution.** The session store hands back the conversation history for
   that key. See [State and sessions](state-and-sessions.md).
...
5. **Prompt assembly.** System prompt, context files, memory and skills are composed
   ...
   [Agent core § Prompt assembly](agent-core.md#prompt-assembly-and-why-it-is-frozen).
...
[Config and profiles](config-and-profiles.md).

## Related

[Agent core](agent-core.md) · [Tools](tools.md) · [Config and profiles](config-and-profiles.md) · [Index](index.md)
