...
dicts in their `plugin.yaml` instead of editing core. See [Gateway](gateway.md#adding-a-platform).
...
- **Gateway adapters that share a credential need scoped locks**, or two profiles
  will connect with the same token. See [Gateway](gateway.md#multi-profile-safety).
...
[Architecture](architecture.md) · [CLI](cli.md) · [Gateway](gateway.md) · [State and sessions](state-and-sessions.md) · [Index](index.md)
